...
5. 如包裹确认丢失，可申请全额退款或补发""",
    ...
    # ─── 发货时效与配送范围 ───
    kb.add_document(KnowledgeDocument(
        doc_id="faq_logistics_002",
        title="发货时效与配送范围",
        content="""发货时效与配送范围说明:
1. 普通订单: 付款后1-3个工作日内发货
2. 预售/定制商品: 以商品页标注时间为准
3. 大促期间(618/双11): 可能延迟2-5天发货
4. 配送范围: 支持全国配送(含港澳台需另询)
5. 北京/上海/广州/深圳等一线城市: 通常发货后1-2天到达
6. 其他城市: 通常发货后2-5天到达
7. 偏远地区(西藏/新疆等): 可能需5-7天
8. 如需加急发货，可联系客服备注优先处理""",
        layer=KnowledgeLayer.FAQ,
        category="物流",
        tags=["发货", "时效", "配送", "范围", "几天到", "北京", "全国"],
        source="FAQ库-物流类#006",
        preset_questions=[
            "多久发货",
            "什么时候发货",
    ...
    # ─── 包邮与运费规则 ───
    kb.add_document(KnowledgeDocument(
        doc_id="faq_logistics_003",
        title="包邮与运费规则",
        content="""包邮与运费规则:
1. 满99元包邮(偏远地区满199元)
2. 偏远地区定义: 西藏、新疆、青海、内蒙古
3. 不满包邮金额: 统一收取运费8元
4. 大件商品(冰箱/洗衣机等): 单独计费，以商品页标注为准
5. VIP/SVIP会员: 全场免运费(每月3次/无限次)
6. 退货运费: 7天无理由退货由买家承担，质量问题由商家承担""",
        layer=KnowledgeLayer.FAQ,
        category="物流",
        tags=["包邮", "运费", "邮费", "免运费", "偏远"],
        source="FAQ库-物流类#007",
        preset_questions=[
            "包邮吗",
            "运费多少",
